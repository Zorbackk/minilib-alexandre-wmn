...
const genererNumeroAdherent = async () => {
  ...
  const count = Number.parseInt(result.rows[0].count) + 1; // Nécessité de parser car PostgreSQL renvoie toujours une string
...
export const create = async ({
  ...
  prenom,
  email,
}: CreateAdherentDto): Promise<Adherent> => {
...
export const updateAdherent = async (
  id: number,
  data: Partial<UpdateAdherentDto>,
